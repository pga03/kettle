...
kettle.dataSource.defaultiseOptions = function (componentOptions, options, directModel, isSet) {
    fluid.log("fluid.dataSource.defaultiseOptions: entering");
    fluid.log("fluid.dataSource.defaultiseOptions: componentOptions: " + JSON.stringify(componentOptions));
    fluid.log("fluid.dataSource.defaultiseOptions: options: " + JSON.stringify(options));
    fluid.log("fluid.dataSource.defaultiseOptions: directModel: " + JSON.stringify(directModel));
    fluid.log("fluid.dataSource.defaultiseOptions: isSet: " + isSet);
    options = options || {};
    ...
    options.auth = options.auth || componentOptions.auth; // Let options sent to function override component options
    return options;
};
...
kettle.dataSource.URL.handle.http = function (path, options, data) {
    fluid.log("kettle.dataSource.URL.handle.http: entering");
    fluid.log("kettle.dataSource.URL.handle.http: path: " + JSON.stringify(path, null, 2));
    fluid.log("kettle.dataSource.URL.handle.http: options: " + JSON.stringify(options, null, 2));
    fluid.log("kettle.dataSource.URL.handle.http: data: " + JSON.stringify(data, null, 2));

    fluid.log("kettle.dataSource.URL.handle.http: creating requestOptions");
    var promise = fluid.promise(),
        ...
        httpCallback = function (res) {
            fluid.log("kettle.dataSource.URL.handle.http->httpCallback");
            var received = "";
            res.setEncoding("utf8");
            res.on("data", function onData(chunk) {
                fluid.log("kettle.dataSource.URL.handle.http->httpCallback->data: getting data chunk");
                received += chunk;
            });
            res.on("end", kettle.wrapCallback(
                function onEnd() {
                    fluid.log("kettle.dataSource.URL.handle.http->httpCallback->end: received: " + JSON.stringify(received, null, 2));;
                    //TODO: if received.error && received.reason, then a Cloudant DB connection was unsuccessful and should trigger
                    //      promise.reject.
    ...
    fluid.log("kettle.dataSource.URL.handle.http: urlObj: " + JSON.stringify(urlObj, null, 2));
    fluid.log("kettle.dataSource.URL.handle.http: requestOptions: " + JSON.stringify(requestOptions, null, 2));

    if (options.operation === "set") {
        fluid.log("kettle.dataSource.URL.handle.http: setting headers. requestOptions before: " + JSON.stringify(requestOptions, null, 2));
        ...
        fluid.log("kettle.dataSource.URL.handle.http: setting headers. requestOptions after: " + JSON.stringify(requestOptions, null, 2));
    }


    if (options.auth) {
        fluid.log("kettle.dataSource.URL.handle.http: adding options.auth to requestOptions.auth");
        requestOptions.auth = options.auth;
    }

    if (/https/i.test(urlObj.protocol)) {
        fluid.log("kettle.dataSource.URL.handle.http: calling https.request with requestOptions: " + JSON.stringify(requestOptions, null, 2));
        var req = https.request(requestOptions, kettle.wrapCallback(httpCallback));
    } else {
        fluid.log("kettle.dataSource.URL.handle.http: calling http.request with requestOptions: " + JSON.stringify(requestOptions, null, 2));
